    ...
    <main className="min-h-screen bg-slate-50">
      <header className="bg-blue-900 text-white px-4 py-5 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Link href="/members" className="bg-blue-700 text-white px-3 py-2 rounded-lg text-base hover:bg-blue-600">
            👥 멤버
          </Link>
          <Link href="/meetings/new" className="bg-white text-blue-900 px-3 py-2 rounded-lg text-base font-semibold hover:bg-blue-50">
            ＋ 새 기록
          </Link>
        </div>
      </header>
      ...
      <div className="max-w-lg mx-auto p-4">
        {loading ? (
          ...
          <div className="text-center text-slate-400 mt-16">
            ...
            <Link href="/meetings/new" className="text-blue-500 text-base mt-2 block">첫 모임 기록하기</Link>
          </div>
        ) : (
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-blue-900 to-blue-700 px-4 py-3">
        ...
        <div className="text-blue-200 text-base mt-0.5">{m.title}</div>
        <div className="flex flex-wrap gap-1 mt-2">
          {m.ulj_attendees?.map((a: any) => (
            <span key={a.ulj_members.id} className="text-sm bg-white/20 text-white px-2 py-0.5 rounded-full">
      ...
      <div className="p-4 space-y-3">
        {m.location && (
          <div className="flex gap-2 items-center">
            <span>📍</span>
            <span className="text-base text-slate-600">{m.location}</span>
          </div>
        )}
        {m.content && (
          <p className="text-base text-slate-500 whitespace-pre-wrap">{m.content}</p>
        )}
        {m.food_name && (
          <div className="flex gap-2">
            <span>🍽</span>
            <div>
              <div className="text-sm text-blue-700 font-medium">음식</div>
              <div className="font-medium text-base">{m.food_name}</div>
              {m.food_review && <div className="text-sm text-slate-500 mt-0.5">{m.food_review}</div>}
            </div>
          </div>
        )}
        {m.total_cost && (
          <div className="flex gap-2 items-center">
            <span>💰</span>
            <div>
              <div className="text-sm text-blue-700 font-medium">비용</div>
              <div className="font-bold text-base">{m.total_cost.toLocaleString()}원</div>
            </div>
          </div>
        )}
      </div>
      ...
      {(foodPhotos.length > 0 || restaurantPhotos.length > 0 || peoplePhotos.length > 0) && (
        <div className="px-4 pb-2 space-y-3">
          {foodPhotos.length > 0 && (
            <div>
              <div className="text-sm text-blue-700 font-medium mb-1">📷 음식 사진</div>
          ...
          {restaurantPhotos.length > 0 && (
            <div>
              <div className="text-sm text-blue-700 font-medium mb-1">🏪 식당 외관</div>
              <div className="grid grid-cols-3 gap-1">
          ...
          {peoplePhotos.length > 0 && (
            <div>
              <div className="text-sm text-blue-700 font-medium mb-1">🤳 단체 사진</div>
              <div className="grid grid-cols-3 gap-1">
      ...
      {m.ulj_moments && m.ulj_moments.length > 0 && (
        <div className="px-4 pb-3">
          <div className="text-sm text-blue-700 font-medium mb-2">💬 오늘의 명언</div>
          <div className="space-y-2">
            {m.ulj_moments.map((moment: any, i: number) => (
              <div key={i} className="bg-amber-50 border-l-4 border-amber-400 rounded-r-xl px-3 py-2">
                <div className="text-sm font-bold text-amber-700 mb-1">{moment.ulj_members?.name}</div>
                <div className="text-base text-slate-600">{linkify(moment.content)}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="px-4 pb-4 flex gap-2">
        <Link href={`/meetings/${m.id}/edit`}
          className="flex-1 text-center text-blue-700 text-base border border-blue-200 py-2 rounded-lg hover:bg-blue-50">
          ✏️ 수정
        </Link>
        <button onClick={handleDelete}
          className="flex-1 text-red-400 text-base border border-red-200 py-2 rounded-lg hover:bg-red-50">
          🗑 삭제
        </button>
      </div>
    </div>
